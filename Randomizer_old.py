...
if not os.path.exists(translation_file_name):
	...
	translation_file_name = translation_catalog + app_name + '_' + default_locale + '.lng'
if not os.path.exists(translation_file_name):
	f = file(translation_file_name, 'w')
	f.close()
translation_file = file(translation_file_name, 'r+')
translations = {}
...
class UserIEventReceiver(IEventReceiver):
	KeyIsDown = {}
	for key in xrange(KEY_KEY_CODES_COUNT):
		KeyIsDown[key] = False
	NumbersKeys = {}
	value = 0
	for key in xrange(KEY_KEY_0, KEY_KEY_9):
		NumbersKeys[key] = str(value)
		value += 1
	value = 0
	for key in xrange(KEY_NUMPAD0, KEY_NUMPAD9):
		NumbersKeys[key] = str(value)
		value += 1
	answer = ''
...
if device:
	...
	def texture_generator(image_format = ECF_R8G8B8, image_size = dimension2du(2, 2), texture_name = 'texture_01', alpha_value = 128, red = (0, 255), green = (0, 255), blue = (0, 255)):
		...
		if image.getColorFormat() in (ECF_A1R5G5B5, ECF_A8R8G8B8, ECF_A16B16G16R16F, ECF_A32B32G32R32F):
			alpha = alpha_value
			blend = True
		for row in xrange(image_size.Height):
			for column in xrange(image_size.Width):
	...
	selector_top = scene_manager.createOctreeTriangleSelector(i_mesh_scene_node_top.getMesh())
	selector_bottom = scene_manager.createOctreeTriangleSelector(i_mesh_scene_node_bottom.getMesh())
	...
	# ADD MAGIC VOLUME
	# i_scene_node = scene_manager.addSphereSceneNode(40.0, id = 1, position = vector3df(0,60,-100))
	i_scene_node = scene_manager.addSphereSceneNode(40.0, id = 1, position = vector3df(0,60,0))
	if i_scene_node:
		# print i_scene_node.getBoundingBox()
		# i_scene_node.setScale(vector3df(10,10,10))
		# i_scene_node.setPosition(vector3df(0,60,100))
		# i_scene_node.updateAbsolutePosition()
		# print i_scene_node.getBoundingBox()
		file_name = 'media//wall.jpg'
		if os.path.exists(file_name) and texture_from_file:
	...
	anim = scene_manager.createCollisionResponseAnimator(selector_bottom, camera)
	camera.addAnimator(anim)
	anim = scene_manager.createCollisionResponseAnimator(i_meta_triangle_selector, camera)
	camera.addAnimator(anim)
	#~ anim = scene_manager.createCollisionResponseAnimator(selector_magic, camera)
	#~ camera.addAnimator(anim)
	anim.drop()
	...
	while result:
			...
			if not dlg:
				...
				if i_event_receiver.IsKeyDown(KEY_RETURN):
					...
					if str(a * b) != i_event_receiver.answer:
						...
						i_gui_static_text.setDrawBackground()
		...
		try:
			...
			print loops_count, 'LAST RUN ERROR', e
			result = False
	device.drop()
else:
