...
def createToolBox():
	...
	wnd = env.addWindow(recti(600,45,800,480), False, "Toolset", None, GUI_ID_DIALOG_ROOT_WINDOW)
...
class MyEventReceiver(IEventReceiver):
	def OnEvent(self, event):
		...
		if event.EventType == EET_KEY_INPUT_EVENT and not event.KeyInput.PressedDown:
			#~ Device.getLogger().log('=EET_KEY_INPUT_EVENT')
			if self.OnKeyUp(self.KeyInput_Key(event.KeyInput)):
				return True
		if (event.EventType == EET_GUI_EVENT) and Device:
...
def main():
	...
	smgr.getParameters().setAttribute("COLLADA_CreateSceneInstances", True)

	driver.setTextureCreationFlag(ETCF_ALWAYS_32_BIT, True)

	smgr.addLightSceneNode(None, vector3df(200,200,200), SColorf(1.0,1.0,1.0), 2000)
	...
	postext = env.addStaticText("", recti(10,50,470,80),False, False, None, GUI_ID_POSITION_TEXT)
